...
#pragma pack (push, 4)
    ...
    public:
        ...
        */
        ...
        GenericValue(const Ch *s, SizeType length, Allocator &allocator) {
            SetStringRaw(s, length, allocator);
        }
                ...
                switch (flags_) {
                    case kArrayFlag:
                        for (GenericValue *v = data_.a.elements;
                             v != data_.a.elements + data_.a.size; ++v)
                            v->~GenericValue();
                        Allocator::Free(data_.a.elements);
                        break;

                    case kObjectFlag:
                        for (Member *m = data_.o.members;
                             m != data_.o.members + data_.o.size; ++m) {
                            m->name.~GenericValue();
        ...
        */
        ...
        const GenericValue &operator[](
                const Ch *name) const { return const_cast<GenericValue &>(*this)[name]; }
        ...
        */
        GenericValue &AddMember(GenericValue &name, GenericValue &value, Allocator &allocator) {
            ...
            if (o.size >= o.capacity) {
                if (o.capacity == 0) {
                    ...
                    o.capacity *= 2;
                    o.members = (Member *) allocator.Realloc(o.members,
        ...
        GenericValue &AddMember(const Ch *name, Allocator &nameAllocator, GenericValue &value,
                                Allocator &allocator) {
    ...
    \endcode
        */
        ...
        const GenericValue &operator[](
                SizeType index) const { return const_cast<GenericValue &>(*this)[index]; }
        ...
        */
        GenericValue &Reserve(SizeType newCapacity, Allocator &allocator) {
            ...
            if (newCapacity > data_.a.capacity) {
                data_.a.elements = (GenericValue *) allocator.Realloc(data_.a.elements,
                                                                      data_.a.capacity *
                                                                      sizeof(GenericValue),
                                                                      newCapacity *
                                                                      sizeof(GenericValue));
        ...
        */
        GenericValue &PushBack(GenericValue &value, Allocator &allocator) {
            ...
            if (data_.a.size >= data_.a.capacity)
                Reserve(data_.a.capacity == 0 ? kDefaultArrayCapacity : data_.a.capacity * 2,
                        allocator);
        ...
        */
        GenericValue &SetString(const Ch *s, Allocator &allocator) {
            SetString(s, internal::StrLen(s), allocator);
            return *this;
        }
    ...
    private:
        ...
        Member *FindMember(const Ch *name) {
            ...
            for (Member *member = o.members; member != data_.o.members + data_.o.size; ++member)
                if (length == member->name.data_.s.length &&
                    memcmp(member->name.data_.s.str, name, length * sizeof(Ch)) == 0)
                    return member;
        ...
        const Member *FindMember(
                const Ch *name) const { return const_cast<GenericValue &>(*this).FindMember(name); }
...
*/
        ...
        */
        ...
        GenericDocument &ParseStream(Stream &stream) {
            ...
            if (reader.template Parse<parseFlags>(stream, *this)) {
                RAPIDJSON_ASSERT(
                        stack_.GetSize() == sizeof(ValueType)); // Got one and only one root object
                this->RawAssign(
                        *stack_.template Pop<ValueType>(1));    // Add this-> to prevent issue 13.
                parseError_ = 0;
                errorOffset_ = 0;
            }
            else {
    ...
    private:
        ...
        void EndObject(SizeType memberCount) {
            typename ValueType::Member *members = stack_.template Pop<typename ValueType::Member>(
                    memberCount);
