...
namespace spatial {
    ...
    public:
        ...
        explicit box_multimap(dimension_type dim)
                : base_type(details::Dynamic_rank(dim)) { except::check_even_rank(dim); }

        box_multimap(dimension_type dim, const Compare &compare)
                : base_type(details::Dynamic_rank(dim), compare) { except::check_even_rank(dim); }
        ...
        box_multimap(dimension_type dim, const Compare &compare,
                ...
                : base_type(details::Dynamic_rank(dim), compare, policy,
                            alloc) { except::check_even_rank(dim); }
