...
namespace spatial {
    ...
    public:
        ...
        explicit idle_point_multiset(dimension_type dim)
                : base_type(details::Dynamic_rank(dim)) { except::check_rank(dim); }

        idle_point_multiset(dimension_type dim, const Compare &compare)
                : base_type(details::Dynamic_rank(dim), compare) { except::check_rank(dim); }
        ...
        idle_point_multiset(dimension_type dim, const Compare &compare,
                            const Alloc &alloc)
                : base_type(details::Dynamic_rank(dim), compare, alloc) { except::check_rank(dim); }
