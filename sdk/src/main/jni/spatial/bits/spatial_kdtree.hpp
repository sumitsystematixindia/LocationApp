...
namespace spatial {
    namespace details {
        ...
        private:
            ...
            key_compare &get_compare() { return _impl._count.base(); }
        ...
        public:
             ...
             */
            rank_type rank() const { return *static_cast<const Rank *>(&_impl); }

            /**
             *  Returns the dimension of the tree.
             */
            dimension_type dimension() const { return rank()(); }

            /**
             *  Returns the compare function used for the key.
             */
            key_compare key_comp() const { return _impl._count.base(); }

            /**
             *  Returns the compare function used for the value.
             */
            value_compare value_comp() const { return value_compare(_impl._count.base()); }
             ...
             */
            size_type max_size() const { return _impl._header.base().max_size(); }
        ...
        inline void swap
                (Kdtree<Rank, Key, Value, Compare, Alloc> &left,
                 Kdtree<Rank, Key, Value, Compare, Alloc> &right) { left.swap(right); }
        ...
        inline bool
        operator>(const Kdtree<Rank, Key, Value, Compare, Alloc> &lhs,
                  const Kdtree<Rank, Key, Value, Compare, Alloc> &rhs) { return rhs < lhs; }
        ...
        inline bool
        operator>=(const Kdtree<Rank, Key, Value, Compare, Alloc> &lhs,
                   const Kdtree<Rank, Key, Value, Compare, Alloc> &rhs) { return !(lhs < rhs); }
        ///@}
                ...
                while (true) {
                    if (key_comp()(node_dim,
                        ...
                        if (node->right != 0) {
                            ...
                            target_node->parent = node;
                            if (node == get_rightmost()) { set_rightmost(target_node); }
                            ++_impl._count();
                            break;
                        }
                    }
        ...
        ::copy_rebalance(const Self &other) {
            ...
            try {
                for (const_iterator i = other.begin(); i != other.end(); ++i) {
                    ptr_store.push_back(create_node(*i));
                } // may throw
            }
            catch (...) {
                for (typename std::vector<node_ptr>::iterator i = ptr_store.begin();
                     i != ptr_store.end(); ++i) { destroy_node(*i); }
                throw;
            }
        ...
        ::rebalance_node_insert
            ...
            do {
                ...
                if (mid + 1 != last) { rebalance_node_insert(mid + 1, last, dim); }
                last = mid;
            }
            while (first != last);
            SPATIAL_ASSERT_CHECK(!empty());
        ...
        Kdtree<Rank, Key, Value, Compare, Alloc>::rebalance() {
            ...
            std::vector<node_ptr> ptr_store;
            ptr_store.reserve(size()); // may throw
            for (iterator i = begin(); i != end(); ++i) { ptr_store.push_back(i.node); }
            _impl.initialize();
            _impl._count() = 0;
            rebalance_node_insert(ptr_store.begin(), ptr_store.end(), 0);
            SPATIAL_ASSERT_CHECK(!empty());
        ...
        Kdtree<Rank, Key, Value, Compare, Alloc>::erase_node
            ...
            while (node->right != 0 || node->left != 0) {
                ...
                if (node->right == 0) {
                    ...
                    node->left = 0;
                    if (get_rightmost() == node) { set_rightmost(maximum(node->right)); }
