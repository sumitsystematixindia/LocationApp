...
namespace spatial {
    ...
    struct tight_balancing {
        ...
        operator()(const Rank &rank, weight_type left, weight_type right) const {
            ...
            if (left < right) { return (right - left > weight); }
            else { return (left - right > weight); }
        }
    };
    ...
    namespace details {
        ...
        private:
            ...
            node_ptr get_header() { return static_cast<node_ptr>(&_impl._header()); }

            const_node_ptr get_header() const { return static_cast<const_node_ptr>(&_impl._header()); }

            node_ptr get_leftmost() { return _impl._leftmost; }

            const_node_ptr get_leftmost() const { return _impl._leftmost; }

            void set_leftmost(node_ptr x) { _impl._leftmost = x; }

            node_ptr get_rightmost() { return _impl._header().right; }

            const_node_ptr get_rightmost() const { return _impl._header().right; }

            void set_rightmost(node_ptr x) { _impl._header().right = x; }

            node_ptr get_root() { return _impl._header().parent; }

            const_node_ptr get_root() const { return _impl._header().parent; }

            void set_root(node_ptr x) { _impl._header().parent = x; }

            rank_type &get_rank() { return *static_cast<Rank *>(&_impl); }

            key_compare &get_compare() { return _impl._compare(); }

            balancing_policy &get_balancing() { return _impl._compare.base(); }

            Link_allocator &get_link_allocator() { return _impl._header.base(); }

            Value_allocator get_value_allocator() const { return _impl._header.base(); }
        ...
        private:
            ...
            struct safe_allocator // RAII for exception-safe memory management
            {
                ...
                safe_allocator(Link_allocator &a)
                        : alloc(&a), link(0) { link = alloc->allocate(1); } // may throw
        ...
        public:
            ...
            iterator end() { return iterator(get_header()); }

            const_iterator end() const { return const_iterator(get_header()); }
        ...
        public:
             ...
             */
            balancing_policy balancing() const { return _impl._compare.base(); }

            /**
             *  Returns the rank type used internally to get the number of dimensions
             *  in the container.
             */
            rank_type rank() const { return *static_cast<const rank_type *>(&_impl); }

            /**
             *  Returns the dimension of the container.
             */
            dimension_type
            dimension() const { return rank()(); }

            /**
             *  Returns the compare function used for the key.
             */
            key_compare key_comp() const { return _impl._compare(); }

            /**
             *  Returns the compare function used for the value.
             */
            value_compare value_comp() const { return value_compare(_impl._compare()); }
             ...
             */
            bool
            empty() const { return (get_root() == get_header()); }
             ...
             */
            size_type
            count() const { return size(); }

            /**
             *  The maximum number of elements that can be allocated.
             */
            size_type
            max_size() const { return _impl._header.base().max_size(); }
             ...
             */
            iterator
            find(const key_type &key) { return equal_begin(*this, key); }

            const_iterator
            find(const key_type &key) const { return equal_begin(*this, key); }
            ///@}
        ...
        public:
             ...
             *  after the copy.
             */
            Relaxed_kdtree(const Relaxed_kdtree &other) : _impl(
                    other._impl) { if (!other.empty()) { copy_structure(other); }}
             ...
             */
            ~Relaxed_kdtree() { destroy_all_nodes(); }
        ...
        public:
            ...
            swap(Self &other) {
                ...
                if (_impl._header().parent !=
                    &_impl._header()) { _impl._header().parent->parent = &_impl._header(); }
                if (other._impl._header().parent !=
                    &other._impl._header()) { other._impl._header().parent->parent = &other._impl._header(); }
            }
            ...
            insert(const value_type &value) {
                ...
                else { return insert_node(0, node, target_node); }
            }
        ...
        operator>(const Relaxed_kdtree
                <Rank, Key, Value, Compare, Balancing, Alloc> &lhs,
                  const Relaxed_kdtree
                          <Rank, Key, Value, Compare, Balancing, Alloc> &rhs) { return rhs < lhs; }
            ...
            while (true) {
                ...
                if (key_comp()(node_dim, const_key(target_node), const_key(node))
                    ...
                    if (node->left == 0) {
                        ...
                        target_node->parent = node;
                        if (get_leftmost() == node) { set_leftmost(target_node); }
                        ++link(node)->weight;
                        break;
                    }
                    else {
                    ...
                    if (node->right == 0) {
                        ...
                        target_node->parent = node;
                        if (get_rightmost() == node) { set_rightmost(target_node); }
                        ++link(node)->weight;
                        break;
                    }
                    else {
        ...
        ::erase_node
            ...
            while (node->right != 0 || node->left != 0) {
                ...
                if (node->left != 0
                    ...
                    if (get_leftmost() == candidate.node) { set_leftmost(node); }
                    if (get_rightmost() == node) { set_rightmost(candidate.node); }
                }
                else {
                    candidate.mapping_dimension() = node_dim;
                    candidate.node_dim = incr_dim(rank(), node_dim);
                    ...
                    if (get_rightmost() == candidate.node) { set_rightmost(node); }
                    if (get_leftmost() == node) { set_leftmost(candidate.node); }
                }
        ...
        ::erase_node_balance
            ...
            while (!header(p)) {
                ...
                --link(p)->weight;
                if (is_node_unbalanced(p)) { p = balance_node(node_dim, p); } // balance node
                p = p->parent;
                node_dim = decr_dim(rank(), node_dim);
            }
        }
