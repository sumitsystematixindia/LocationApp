...
namespace spatial {
    ...
    public:
        ...
        neighbor_iterator<Ct, Metric> &operator++() { return increment_neighbor(*this); }
        ...
        //! use this form in \c for loops.
        neighbor_iterator<Ct, Metric> &operator--() { return decrement_neighbor(*this); }
    ...
    distance(const neighbor_iterator<Ct, Metric> &iter) { return iter.distance(); }

    template<typename Ct, typename Metric>
    inline void
    distance(neighbor_iterator<Ct, Metric> &iter,
             const typename Metric::distance_type &dist) { iter.distance() = dist; }
    ///@}
    ...
    target_key(const neighbor_iterator<Ct, Metric> &iter) { return iter.target_key(); }
    ...
    namespace details {
        ...
        increment_neighbor(neighbor_iterator<Container, Metric> &it) {
            ...
            do {
                ...
                if (tmp < distance(it) || (tmp == distance(it) && rn < curr)) { goto left_side; }
                ...
                if (tmp < distance(it) || (tmp == distance(it) && ln < curr)) { continue; }
                if (near_node == 0 || tmp < near_distance) {
                    near_node = ln;
                    near_dim = ln_dim;
                    near_distance = tmp;
        ...
        decrement_neighbor(neighbor_iterator<Container, Metric> &it) {
            ...
            do {
                ...
                tmp = met.distance_to_key(rank(), target_key(it), const_key(ln));
                if (tmp > distance(it) || (tmp == distance(it) && ln > curr)) { goto right_side; }
                if (near_node == 0 || tmp > near_distance) {
                    near_node = ln;
                    near_dim = ln_dim;
                    near_distance = tmp;
                ...
                tmp = met.distance_to_key(rank(), target_key(it), const_key(rn));
                if (tmp > distance(it) || (tmp == distance(it) && rn > curr)) { continue; }
                if (near_node == 0 || tmp > near_distance) {
                    near_node = rn;
                    near_dim = rn_dim;
                    near_distance = tmp;
